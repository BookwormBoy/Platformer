...
class Level:
    ...
    def scroll_x(self):
        ...
        if player_x < screen_width/3 and player.vel.x<0:
            self.shift_x = -player.vel.x
            player.pos.x += self.shift_x
        elif player_x > 2*screen_width/3 and player.vel.x>0:
            self.shift_x = -player.vel.x 
            player.pos.x += self.shift_x
        else:
            self.shift_x = 0
    ...
    def scroll_y(self):
        ...
        if player.pos.y<screen_height/3 and player.vel.y<0:
            self.shift_y = -player.vel.y
            player.pos.y += self.shift_y
        elif player.pos.y > 2*screen_height/3 and player.vel.y>0:
            self.shift_y = -player.vel.y 
            player.pos.y += self.shift_y
        else:
            self.shift_y=0
